  ...
  <div>
      ...
      <div class="col-5">
        <div class="input-group justify-content-center">
          <div class="input-group">
            ...
            <button class="btn btn-primary" @click="randomizer">
              Generate
            </button>
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-12">
        ...
        <div v-else>
          <p v-for="(item, i) in currentRandomWords" :key="i" class="text-center text-big">
            {{ item }}
          </p>
        </div>
      </div>
...
const currentRandomWords = computed(() => {
  return currentRandomIndexes.value.map((item) =>
    wordType.value === "hiragana"
      ? hiragana[item].kana
      : wordType.value === "katakana"
        ? katakana[item].kana
        : hiragana[item].romaji
  );
});
</script>
